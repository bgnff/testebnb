...
export function useRealtimeColumns() {
  ...
  useEffect(() => {
    ...
    const loadColumns = async () => {
      try {
        console.log('[useRealtimeColumns] Loading all columns (BnBWeb single company)');
      ...
      globalColumnsChannel = supabase
        ...
        .on(
          ...
          (payload) => {
            ...
            // Process all columns (no board filter for single company)
