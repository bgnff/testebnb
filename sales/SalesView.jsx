        ...
        <div className="flex items-center rounded-lg border border-border p-0.5">
          {['all', 'manutencao_recorrente', 'cliente_ativo', 'pendente', 'inativo'].map((s) => (
            <button
              ...
              className={`px-3 py-1.5 text-xs font-medium rounded-md transition-colors ${statusFilter === s ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground'}`}
            >
        ...
        {loading ? (
          ...
          <div className="max-w-4xl mx-auto space-y-2">
            {filtered.map((c) => {
              ...
              return (
                <div key={c.id} className="group flex items-center gap-3 bg-card rounded-xl border border-border px-3 py-2.5 hover:shadow-sm hover:border-primary/30 transition-all">
                  <button onClick={() => setDetail(c)} className="flex items-center gap-3 flex-1 min-w-0 text-left">
                    <span className="w-10 h-10 rounded-xl bg-primary/10 text-primary flex items-center justify-center font-semibold flex-shrink-0">
                      {c.name?.charAt(0).toUpperCase()}
                    </span>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <p className="font-medium truncate">{c.name}</p>
                        <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium flex-shrink-0 ${st.chip}`}>{st.label}</span>
                      </div>
                  ...
                  <div className="flex items-center gap-1 flex-shrink-0">
                    ...
                    <button
                      ...
                      className="w-9 h-9 rounded-lg flex items-center justify-center border border-border text-muted-foreground hover:bg-[#25D366] hover:text-white hover:border-[#25D366] disabled:opacity-40 disabled:hover:bg-transparent disabled:hover:text-muted-foreground disabled:hover:border-border transition-colors"
                    >
                    ...
                    <button
                      ...
                      onClick={() => setDetail(c)}
                      className={`w-9 h-9 rounded-lg flex items-center justify-center border transition-colors ${hasMeeting ? 'bg-primary/10 border-primary/30 text-primary hover:bg-primary hover:text-primary-foreground' : 'border-border text-muted-foreground/40 hover:bg-muted hover:text-muted-foreground'}`}
                    >
